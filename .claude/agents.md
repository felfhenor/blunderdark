...
- **NEVER call `updateGamestate` directly from components** — always create and use helper functions in the appropriate `src/app/helpers/` file. Components should call domain-specific action functions (e.g. `darkForgeCraft()`, `breedingStartJob()`, `trapPlaceAction()`) that internally wrap `updateGamestate`. This keeps state mutation logic in helpers where it can be tested and reused.
...
- **NEVER commit PRD files** — PRD files (e.g. `tasks/prd-*.md`) are planning artifacts only. Do not include them in git commits.
...
- **Aura effectTypes for legendaries**: Use `legendaryAuraGetBonus(inhabitants, effectType)` from `legendary-inhabitant.ts` to sum active aura values. Active auras: `aura_trap_bonus` (traps.ts damage), `aura_reveal_invaders` (traps.ts disarm reduction), `aura_fear_bonus` (fear-level.ts multiplicative), `aura_food_bonus` (production.ts food multiplier), `aura_gathering_bonus` (production.ts all-resource multiplier), `aura_petrify` (invasion-process.ts stun invaders), `aura_negate_scout` (invasion-process.ts blocks scouting), `aura_room_regen` (invasion-process.ts altar damage reduction). Data-only (no system yet): `aura_attack_bonus`, `aura_corruption_bonus`, `aura_morale_penalty`.
...
- **Bonus types**: `capacity_bonus`, `fear_reduction`, `production_bonus`, `adjacent_production`, `flat_production`, `corruption_generation`, `combat_bonus`
...
- **Maximize Tailwind classes in single-use components** — for inline-template components, move as many CSS properties as possible to Tailwind/daisyUI utility classes on DOM elements or `host: { class: '...' }`. Only keep CSS for properties without Tailwind equivalents (custom animations/keyframes, `::ng-deep` overrides, truly unique colors with no daisyUI match). Common mappings: `display: inline-flex` → `inline-flex`, `align-items: center` → `items-center`, `gap: 4px` → `gap-1`, `padding` → `px-*/py-*`, `border-radius` → `rounded*`, `white-space: nowrap` → `whitespace-nowrap`, `cursor: default` → `cursor-default`, `opacity` → `opacity-*`, `backdrop-filter: blur` → `backdrop-blur-*`.
- **Prefer daisyUI/Tailwind color classes over custom CSS color rules:**
  - **Soft colored badges** — use `badge badge-{color} badge-soft badge-outline` instead of manually recreating with `color-mix(in oklch, var(--color-X) 15%, transparent)` background + `color-mix(in oklch, var(--color-X) 40%, transparent)` border. The daisyUI classes produce the same visual effect.
  - **Translucent panel backgrounds** — use Tailwind opacity modifier `bg-base-200/80` instead of `color-mix(in oklch, var(--color-base-200) 80%, transparent)`.
  - **Solid indicator backgrounds** — use `bg-success text-success-content`, `bg-warning text-warning-content`, `bg-error text-error-content` instead of custom CSS classes setting `background-color: var(--color-success)`.
  - **Colored text** — use Tailwind `text-success`, `text-warning`, `text-error`, `text-info` instead of CSS `color: var(--color-success)`.
  - **Colored borders** — use Tailwind `border-warning/40` or `border-base-300/30` instead of `color-mix(in oklch, var(--color-warning) 40%, transparent)`.
  - **Never use old daisyUI 4 variables** — `oklch(var(--wa))`, `oklch(var(--er))`, `oklch(var(--n))`, `oklch(var(--b3))`, `oklch(var(--bc))` etc. are defunct. Use daisyUI 5 `var(--color-warning)` variables or Tailwind classes.
  - **When to keep custom CSS** — only for truly unique colors with no daisyUI equivalent (e.g. `oklch(0.45 0.15 40)` for a burnt-orange fear level, `oklch(0.7 0.15 50)` for high corruption), complex gradients, and animation keyframes.
...
- **Export**: `.blunder` files now contain `SaveData` (not raw `GameState`); import detects both formats via `saveParseLegacy`
