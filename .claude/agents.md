...
- **Trait effectTypes**: `production_bonus` (with `targetResourceType`) — **only actively consumed type** in production.ts/efficiency.ts. `attack_bonus` and `defense_bonus` consumed by mimic.ts for combat trait effects. Others are data-only: `trap_bonus`, `versatility`, `damage_reduction`, `detection`, `structural_bonus`, `random_event`, `fire_damage_bonus`, `corruption_reduction`, `corruption_generation`, `regeneration`, `fear_immunity`, `fear_bonus`, `physical_evasion`, `training_bonus`, `combat_priority`
- **Room-targeted traits**: use `targetRoomName` field on InhabitantTrait to make traits conditional on room assignment (e.g., Treasure Guardian applies only in Treasure Vault). Code in `mimic.ts` resolves room name via `contentGetEntry`.
