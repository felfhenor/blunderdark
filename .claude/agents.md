...
- **Trait effectTypes**: `production_bonus` (with `targetResourceType`) — **only actively consumed type** in production.ts/efficiency.ts. Others are data-only: `trap_bonus`, `defense_bonus`, `versatility`, `damage_reduction`, `detection`, `structural_bonus`, `random_event`, `fire_damage_bonus`, `corruption_reduction`, `corruption_generation`, `regeneration`, `fear_immunity`, `fear_bonus`, `physical_evasion`, `training_bonus`, `attack_bonus`, `combat_priority`
