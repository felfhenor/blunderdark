...
describe('rollD20', () => {
  it('should return 1 for rng() = 0.0', () => {
    expect(rollD20(fixedRng(0.0))).toBe(1);
  });
...
describe('resolveCombat', () => {
  it('should return a hit with damage when roll succeeds', () => {
    ...
    const result = resolveCombat(attacker, defender, rngForRoll(10));
  ...
  it('should return a miss with 0 damage when roll fails', () => {
    ...
    const result = resolveCombat(attacker, defender, rngForRoll(3));
  ...
  it('should always hit on natural 20', () => {
    ...
    const result = resolveCombat(attacker, defender, rngForRoll(20));
    expect(result.hit).toBe(true);
  ...
  it('should always miss on natural 1', () => {
    ...
    const result = resolveCombat(attacker, defender, rngForRoll(1));
    expect(result.hit).toBe(false);
  ...
  it('should use injectable RNG for deterministic results', () => {
    const attacker = makeUnit({ attack: 10 });
    const defender = makeUnit({ defense: 8, hp: 30, maxHp: 30 });
    const result1 = resolveCombat(attacker, defender, rngForRoll(15));
    const result2 = resolveCombat(attacker, defender, rngForRoll(15));
